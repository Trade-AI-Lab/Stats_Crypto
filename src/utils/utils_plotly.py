...
def multiple_signals_same_graph(X: Union[list, np.array], dict_Y: dict):
    ...
    for keys in list(dict_Y):
        fig.add_trace(go.Scatter(x=X, y=dict_Y[keys], mode="lines", name=str(keys)))
    fig.show()


def subplots_plotly(X: Union[list, np.array], dict_Y: dict, browser=None):
    """
    ...
    fig = sp.make_subplots(
        rows=len(dict_Y), cols=1, shared_xaxes=True, vertical_spacing=0.05
    )

    n = 1
    for keys in list(dict_Y):
        fig.add_trace(
            go.Scatter(x=X, y=dict_Y[keys], mode="lines", name=keys, showlegend=False),
            row=n,
            col=1,
        )
        n += 1
    ...
    fig.show("browser") if browser else fig.show()
...
def subplots_plotly_markers(X, dictionnary):
    ...
    fig = sp.make_subplots(
        rows=len(dictionnary), cols=1, shared_xaxes=True, vertical_spacing=0.05
    )
    n = 1
    for key in dictionnary.keys():
        ...
        line_signal = go.Scatter(x=X, y=signal, name=f"{key}_signal", mode="lines")
